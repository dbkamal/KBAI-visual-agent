...
class Agent:
    ...
    # Returning your answer as a string may cause your program to crash. 0
    ...
    def solve2x2(self, problem):
        # Determine existing relational ratios
        aArray = self.getImageArray(problem, 'A')
        bArray = self.getImageArray(problem, 'B')
        cArray = self.getImageArray(problem, 'C')
        ...
        solVertRatios = {}
        solNames = []
        # Collect ratios to possible solutions and set of solutions
        for solName in sorted(problem.figures):
            if solName.isnumeric():
                solArray = self.getImageArray(problem, solName)
                solHorizRatios[solName] = self.getRatioDarkPixelsQuads(cArray, solArray)
                solVertRatios[solName] = self.getRatioDarkPixelsQuads(bArray, solArray)
                solNames.append(solName)
                solImageArrays[solName] = solArray

        # Determine best horizontal solution
        self.getClosestRatio(solHorizRatios, abPixelRatio)

        # Determine best vertical solution
        self.getClosestRatio(solVertRatios, acPixelRatio)
        ...
        if self.isSameFigure(a, b):
            for sol in solNames:
                if self.isSameFigure(c, self.centerImageArray(solImageArrays[sol])):
                    self.vote(sol, self.identicalVote)
        if self.isSameFigure(a, c):
            for sol in solNames:
                if self.isSameFigure(b, self.centerImageArray(solArray)):
                    self.vote(sol, self.identicalVote)


        #Is a left/right figure flip?
        #Horizontal
        if self.isLeftRightFlip(aArray, bArray):
            for sol in solNames:
                if self.isLeftRightFlip(cArray, solImageArrays[sol]):
                    self.vote(sol, self.flipVote)

        #Vertical
        if self.isLeftRightFlip(aArray, cArray):
            for sol in solNames:
                if self.isLeftRightFlip(bArray, solImageArrays[sol]):
                    self.vote(sol, self.flipVote)

        #Is an up/down figure flip?
        if self.isUpDownFlip(aArray, bArray):
            for sol in solNames:
                if self.isUpDownFlip(cArray, solImageArrays[sol]):
                    self.vote(sol,self.flipVote)

        #Vertical
        if self.isUpDownFlip(aArray, cArray):
            for sol in solNames:
                if self.isUpDownFlip(bArray, solImageArrays[sol]):
                    self.vote(sol, self.flipVote)
    ...
    def solve3x3(self, problem):
        ...
        dArray = self.getImageArray(problem, 'D')
        eArray = self.getImageArray(problem, 'E')
        fArray = self.getImageArray(problem, 'F')
        gArray = self.getImageArray(problem, 'G')
        hArray = self.getImageArray(problem, 'H')
